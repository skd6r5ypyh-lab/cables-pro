...
function blankReport(){
 ...
 photos:[],customerSignedBy:'',customerSignature:'',engineerSignature:'',emailSent:false};
}
function shell(body){
 return `<div class="app"><header><div class="brand"><img src="logo.png"><div><b>Cables Pro</b><small>Professional Intruder Alarm Edition</small></div></div><div class="version">V9</div></header><main>${body}</main>
...
function diary(){
 ...
 return `<div class="section"><h2>Diary</h2><button class="btn primary" data-new-job>＋ Book Job</button></div>${rows.map(j=>`<div class="card"><div class="row"><div class="avatar">${esc(j.time)}</div><div class="grow"><b>${esc(j.customer)}</b><div class="muted">${esc(j.date)} · ${esc(j.title)}</div></div><span class="badge ${j.status==='Complete'?'good':''}">${esc(j.status)}</span></div><button class="btn full" style="margin-top:11px" data-edit-job="${j.id}">Open</button></div>`).join('')||'<div class="card empty">No diary appointments.</div>'}`;
}
function customers(){
...
function reportForm(){
 ...
 <label class="field">Overall result<select data-field="result">${['Satisfactory','Satisfactory with recommendations','Further action required','System left out of service'].map(x=>`<option ${x===r.result?'selected':''}>${x}</option>`).join('')}</select></label></div>`,
 ...
 <div class="card"><h3>Customer signature</h3>${field('Customer / responsible person','customerSignedBy',r.customerSignedBy)}<canvas class="signature" id="customerCanvas" width="900" height="260"></canvas><button class="btn" data-clear-signature="customer">Clear customer signature</button></div>
 <div class="card"><h3>Engineer signature</h3><canvas class="signature" id="engineerCanvas" width="900" height="260"></canvas><button class="btn" data-clear-signature="engineer">Clear engineer signature</button></div>`
 ];
 return `<div class="section"><div><div class="eyebrow">Intruder Alarm Service</div><h2 style="margin:4px 0">${esc(r.number)}</h2></div><span class="badge">${esc(r.status)}</span></div><div class="notice">Draft is saved automatically every 15 seconds. Use the Previous and Next buttons to move through the report.</div>
...
function preview(){
 ...
 ${line('Customer',r.customer)}${line('Site address',r.address)}${line('Contact',r.contact)}${line('Service date',r.date)}${line('Visit type',r.visit)}${line('Engineer',r.engineer)}${line('Time on site',r.arrival+'–'+r.departure)}
 ...
 <h3>Overall result</h3><div class="result">${esc(r.result)}</div>${r.photos.length?`<h3>Photographs</h3><div class="photo-grid">${r.photos.map(p=>`<div class="photo"><img src="${p}"></div>`).join('')}</div>`:''}
 <h3>Customer sign-off</h3><p>${esc(r.customerSignedBy||'Not recorded')}</p>${r.customerSignature?`<img class="signature-preview" src="${r.customerSignature}">`:''}<h3>Engineer sign-off</h3>${r.engineerSignature?`<img class="signature-preview" src="${r.engineerSignature}">`:'<p>Not signed</p>'}
...
function canvasSetup(id,key){
 ...
 const start=e=>{on=true;last=p(e);e.preventDefault()},move=e=>{if(!on)return;const q=p(e);ctx.beginPath();ctx.moveTo(last.x,last.y);ctx.lineTo(q.x,q.y);ctx.stroke();last=q;e.preventDefault()},end=()=>{if(on){on=false;current[key]=c.toDataURL()}};
...
function bind(){
 ...
 document.querySelectorAll('[data-complete]').forEach(b=>b.onclick=()=>{sync();if(!current.customer.trim()){toast('Enter the customer name first');return}persist('Complete');route='preview';render()});
 ...
 document.querySelectorAll('[data-clear-signature]').forEach(b=>b.onclick=()=>{current[b.dataset.clearSignature==='customer'?'customerSignature':'engineerSignature']='';render()});
