...
function blankReport(){
 ...
 photos:[],customerSignedBy:'',customerSignature:'',engineerSignature:'',emailSent:false,linkedJobId:''};
}
function shell(body){
 return `<div class="app"><header><div class="brand"><img src="logo.png"><div><b>Cables Pro</b><small>Professional Intruder Alarm Edition</small></div></div><div class="version">V9.1</div></header><main>${body}</main>
...
function bind(){
 ...
 document.querySelectorAll('[data-complete]').forEach(b=>b.onclick=()=>{sync();if(!current.customer.trim()){toast('Enter the customer name first');return}persist('Complete');if(current.linkedJobId){const j=data.jobs.find(x=>x.id===current.linkedJobId);if(j){j.status='Complete';save(KEY_DATA,data)}}route='preview';render()});
