...
function renderHome(){
  ...
  const dueSoon=cs.filter(c=>c.nextService && c.nextService>=today && c.nextService<=inSeven.toISOString().slice(0,10));
  const todayJobs=js.filter(j=>j.serviceDate===today);
  const awaiting=js.filter(j=>j.status==='Draft');
  ...
  <div class="stats">
    <div class="card stat"><strong>${todayJobs.length}</strong><span>Jobs Today</span></div>
    <div class="card stat"><strong>${cs.length}</strong><span>Customers</span></div>
    <div class="card stat"><strong>${dueSoon.length}</strong><span>Due This Week</span></div>
    <div class="card stat"><strong>${awaiting.length}</strong><span>Drafts</span></div>
  </div>
  ...
  <div class="section-head"><h3>Today's jobs</h3></div>
  <div class="card">${todayJobs.map(j=>jobRow(j)).join('')||'<p class="muted">No jobs dated for today.</p>'}</div>
  <div class="section-head"><h3>Service reminders</h3></div>
  <div class="card">${dueSoon.map(c=>`<div class="row"><div class="avatar">📅</div><div class="grow"><b>${escapeHtml(c.name)}</b><small class="muted">${escapeHtml(c.address)}</small></div><span class="badge warn">${new Date(c.nextService).toLocaleDateString('en-GB')}</span></div>`).join('')||'<p class="muted">No services due in the next seven days.</p>'}</div>
...
function renderReport(){
  ...
  <h3>Summary</h3>${kv('Overall result',j.overall)}${kv('Faults found',j.faults||'None recorded')}${kv('Recommendations',j.recommendations||'None recorded')}${kv('Parts used',j.partsUsed||'None recorded')}${kv('Next service due',j.nextService)}
